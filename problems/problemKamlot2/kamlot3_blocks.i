[Mesh]
  [efm]
   ...
   file = kamlot3new.msh  #kamlot3newR.msh  #kamlot5.msh
  []
[]
...
#   block = 'casing rocksalt fluid injection_region'
 ...
 []
  ...
  [elasticity_tensor_fluid]
    type = ComputeIsotropicElasticityTensor
    youngs_modulus = 1e6  #1e6           #1e-9
    poissons_ratio = 0.0   #0.0                        
    block = 'fluid injection_region'
  []
  [strain]
...
[BCs]
  [symmetric_face_left_fix_u]
   ...
   variable = disp_x
    value = 0
    boundary = 'left' # casingL fluidL injection_areaL'        
  []
  [symmetric_face_front_fix_u]
   type = DirichletBC
   variable = disp_y
    value = 0
    boundary = 'front' # casingF fluidF injection_areaF'        
  []
    [bottom_face_fix_u]
    type = DirichletBC
    variable = disp_z
...
#  []
  [external_boundaries_atm_pressure] 
    ...
    value = 101325
    use_displaced_mesh = false
    boundary = 'right back top bottom' # casingT fluidT'
  []
  ...
  [injection_compressive_stress_x]
    type = Pressure
    boundary = 'injection_area injection_areaF'
    variable = disp_x
    use_displaced_mesh = false
  ...
  [injection_compressive_stress_y]
    type = Pressure
    boundary = 'injection_area injection_areaL'
    variable = disp_y
    use_displaced_mesh = false
    postprocessor = constrained_effective_fluid_pressure_at_wellbore
  []
