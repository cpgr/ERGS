...
[BCs]
  [symmetric_face_left_fix_u]
    ...
    variable = disp_x
    value = 0
    boundary = right #'left'                        #right to mimick 3d      
  []
 [bottom_face_fix_u]
   type = DirichletBC
   variable = disp_y
  ...
  []
    ...
    type = Pressure
    function = 12e6             #21e6                                            12e6 #converge
    variable = disp_x
    use_displaced_mesh = false
    boundary = left #'right'                         #left to mimick 3d 
  []
...
[Executioner]
  ...
  []
   nl_abs_tol = 1e-9                                                                             #1e-11/10 converge for flux1/2
[]
