...
[Materials]
  ...
  []
    [permeability_rocksalt]
     ...
     N = "1 0 0  0 1 0  0 0 1"
     km = 1e-21 
     alpha = " 0.024 0.024 0.024"                   # "1e-2 1e-2 1e-2" 
     eps0 =  "-3e-5 0 0  0 -3e-5 0  0 0 -3e-5"      # "1e-5 1e-5 1e-5"      "0 0 0"
    block = rocksalt
  []
    [permeability_casing]
     type = PFOrthoEM
...
#  []
  ...
  [water_injection]
    type = PorousFlowSink
    boundary = 'injectionT injectionB injectionR'            
    variable = pwater
...
[Functions]
  [my_flux1]
  type = ParsedFunction 
  value = 'if(t <= 300, -1.39e-7, -2.78e-3)'  #'if(t <= 300,-1.39e-7,-2.78e-6)' 'if(t <= 300, 0,-2.78e-6)'
  []
  [my_flux2]
  type = ParsedFunction 
  value = 'if(t <= 300, 0.0, -2.78e-5)'
  []
  [pcw_flux1]
  ...
  [pcw_flux2]
    type = PiecewiseLinear
    x = '240   2400' 
    y = '-5.5e-7 -2.78e-5'      #'-5.5e-7 -2.78e-5'
  []
